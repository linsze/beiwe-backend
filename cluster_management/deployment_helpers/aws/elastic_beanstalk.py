...
def construct_eb_environment_variables(eb_environment_name):
    ...
    try:
        environment_variables = get_finalized_settings_variables(
            eb_environment_name)
    ...
    try:
        server_settings = get_server_configuration_variables(
            eb_environment_name)
    except Exception as e:
...
def get_python38_platform_arn():
    ...
    platforms = []
    boto_filters = [{'Operator': 'contains',
                     'Type': 'PlatformName', 'Values': ['Python']}]
    ...
    if len(platforms) == 0:
        raise PythonPlatformDiscoveryError(
            "could not find python 3.8 platform")
    if len(platforms) > 1:
        log.error("\n***********************************************************\n"
...
def get_or_create_eb_service_role():
    ...
    try:
        ...
        iam_create_role(iam_client, EB_SERVICE_ROLE,
...
def get_or_create_eb_instance_profile_role():
    ...
    try:
        ...
        iam_create_role(iam_client, EB_INSTANCE_PROFILE_ROLE,
    ...
    iam_attach_role_policy(
        iam_client, EB_INSTANCE_PROFILE_ROLE, AWS_EB_MULTICONTAINER_DOCKER)
    iam_attach_role_policy(
        iam_client, EB_INSTANCE_PROFILE_ROLE, AWS_EB_WEB_TIER)
    iam_attach_role_policy(
        iam_client, EB_INSTANCE_PROFILE_ROLE, AWS_EB_WORKER_TIER)
...
def allow_eb_environment_database_access(eb_environment_name):
    ...
    eb_sec_grp_name = get_eb_instance_security_group_identifier(
        eb_environment_name)
    eb_sec_grp_id = get_security_group_by_name(eb_sec_grp_name)['GroupId']
    add_eb_environment_to_rds_database_security_group(
        eb_environment_name, eb_sec_grp_id)
...
def create_eb_environment(eb_environment_name, without_db=False):
    ...
    log.info("creating a new Elastic Beanstalk environment named %s... this will take a while." %
             eb_environment_name)
    eb_client = create_eb_client()
    ...
    while True:
        ...
        log.info(
            f'{current_time_string()}: Elastic Beanstalk status is "{env["Status"]}", waiting until status is "Ready"'
        )
        if len(envs) != 1:
            raise Exception(
                f"describe_environments is broken, {len(envs)} environments returned")
        env = envs[0]
        if env['Status'] in bad_eb_environment_states:
            msg = f"environment deployment failed:\n{format(env)}"
        ...
        if env['Status'] == "Ready":
            log.info(
                f"environment {eb_environment_name}, is ready to have Beiwe deployed to it.")
            break
