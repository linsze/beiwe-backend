...
# Queue tasks to run using celery
APT_MANAGER_INSTALLS.append('rabbitmq-server')
...
FILES_TO_PUSH = [
    ...
    # allows git clone without further prompting
...
DEPLOYMENT_SPECIFIC_CONFIG_FOLDER = path_join(
    CLUSTER_MANAGEMENT_FOLDER, 'environment_configuration')
GENERAL_CONFIG_FOLDER = path_join(
    CLUSTER_MANAGEMENT_FOLDER, 'general_configuration')
...
REMOTE_HOME_DIR = path_join('/home', REMOTE_USERNAME).replace("\\", "/")
REMOTE_PROJECT_DIR = path_join(
    REMOTE_HOME_DIR, "beiwe-backend").replace("\\", "/")

# Global EC2 Instance remote file paths
DEPLOYMENT_ENVIRON_SETTING_REMOTE_FILE_PATH = path_join(
    REMOTE_PROJECT_DIR, 'config/remote_db_env.py').replace("\\", "/")
LOG_FILE = path_join(REMOTE_HOME_DIR, 'server_setup.log').replace("\\", "/")

# Management Tool Configuration Files
AWS_CREDENTIALS_FILE = path_join(GENERAL_CONFIG_FOLDER, 'aws_credentials.json')
GLOBAL_CONFIGURATION_FILE = path_join(
    GENERAL_CONFIG_FOLDER, 'global_configuration.json')
AWS_PEM_FILE = path_join(
    DEPLOYMENT_SPECIFIC_CONFIG_FOLDER, 'aws_deployment_key.pem')
...
ELASTICBEANSTALK_ASSUME_ROLE_POLICY_DOCUMENT_PATH = path_join(
    GENERAL_CONFIG_FOLDER, "elasticbeanstalk_assume_role_policy_document.json")
INSTANCE_ASSUME_ROLE_POLICY_DOCUMENT_PATH = path_join(
    GENERAL_CONFIG_FOLDER, "instance_assume_role_policy_document.json")
AUTOMATION_POLICY_PATH = path_join(
    GENERAL_CONFIG_FOLDER, "beiwe_automation_policy.json")
BEIWE_SERVER_AWS_ACCESS_PATH = path_join(
    GENERAL_CONFIG_FOLDER, "beiwe_server_aws_access.json")
...
LOCAL_CRONJOB_WORKER_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'cron_worker.txt').replace("\\", "/")
LOCAL_CRONJOB_MANAGER_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'cron_manager.txt').replace("\\", "/")
LOCAL_CRONJOB_SINGLE_SERVER_AMI_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'cron_ami.txt').replace("\\", "/")
REMOTE_CRONJOB_FILE_PATH = path_join(
    REMOTE_HOME_DIR, 'cronjob.txt').replace("\\", "/")
LOCAL_INSTALL_CELERY_WORKER = path_join(
    PUSHED_FILES_FOLDER, 'install_celery_worker.sh').replace("\\", "/")
REMOTE_INSTALL_CELERY_WORKER = path_join(
    REMOTE_HOME_DIR, 'install_celery_worker.sh').replace("\\", "/")
LOCAL_AMI_ENV_CONFIG_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'ami_env_config.py').replace("\\", "/")
LOCAL_APACHE_CONFIG_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'ami_apache.conf').replace("\\", "/")
REMOTE_APACHE_CONFIG_FILE_PATH = path_join(
    REMOTE_HOME_DIR, 'ami_apache.conf').replace("\\", "/")
LOCAL_RABBIT_MQ_CONFIG_FILE_PATH = path_join(
    PUSHED_FILES_FOLDER, 'rabbitmq_configuration.txt').replace("\\", "/")
REMOTE_RABBIT_MQ_CONFIG_FILE_PATH = path_join(
    REMOTE_HOME_DIR, 'rabbitmq_configuration.txt').replace("\\", "/")
REMOTE_RABBIT_MQ_FINAL_CONFIG_FILE_PATH = '/etc/rabbitmq/rabbitmq-env.conf'
REMOTE_RABBIT_MQ_PASSWORD_FILE_PATH = path_join(
    REMOTE_PROJECT_DIR, "manager_ip").replace("\\", "/")
...
def get_rabbit_mq_manager_ip_file_path(eb_environment_name):
    return path_join(
        DEPLOYMENT_SPECIFIC_CONFIG_FOLDER, eb_environment_name +
        "_" + RABBIT_MQ_PASSWORD_FILE_NAME
    )
...
RDS_INSTANCE_SEC_GROUP_NAME_OVERRIDE = get_env(
    "RDS_INSTANCE_SEC_GROUP_NAME_OVERRIDE", None)
RDS_DATABASE_SEC_GROUP_NAME_OVERRIDE = get_env(
    "RDS_DATABASE_SEC_GROUP_NAME_OVERRIDE", None)
